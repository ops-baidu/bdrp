...
redis_parse_rsp(struct msg *r)
{
    ...
    enum {
        ...
        SW_MULTIBULK_ARGN,
        SW_MULTIBULK_ARGN_BULK,
        SW_MULTIBULK_ARGN_BULK_LEN,
        SW_MULTIBULK_ARGN_BULK_LEN_LF,
        SW_MULTIBULK_ARGN_BULK_START,
        SW_MULTIBULK_ARGN_INTEGER,
        SW_MULTIBULK_ARGN_INTEGER_START,
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_MULTIBULK_ARGN:
            ...
            case '$':
                p = p - 1; /* go back by 1 byte */
                state = SW_MULTIBULK_ARGN_BULK_LEN;
                break;

            case ':':
                p = p - 1; /* go back by 1 byte */
                state = SW_MULTIBULK_ARGN_INTEGER;
                break;
        ...
        case SW_MULTIBULK_ARGN_BULK_LEN:
            if (r->token == NULL) {
                if (ch != '$') {
                    goto error;
                }
                ...
                if (r->rlen == 1 && (p - r->token) == 3) {
                    ...
                    state = SW_MULTIBULK_ARGN_BULK_LEN_LF;
                }
                r->rnarg--;
                r->token = NULL;
        ...
        case SW_MULTIBULK_ARGN_BULK_LEN_LF:
            switch (ch) {
            case LF:
                state = SW_MULTIBULK_ARGN_BULK_START;
                break;
        ...
        case SW_MULTIBULK_ARGN_BULK_START:
            m = p + r->rlen;
            if (m >= b->last) {
                r->rlen -= (uint32_t)(b->last - p);
                m = b->last - 1;
        ...
        case SW_MULTIBULK_ARGN_INTEGER:
            /* rsp_start <- p */
            state = SW_MULTIBULK_ARGN_INTEGER_START;
            r->token = p;
            r->integer = 0;

            break;

        case SW_MULTIBULK_ARGN_INTEGER_START:
            if (ch == CR) {
                state = SW_MULTIBULK_ARGN_LF;
                r->rnarg--;
                r->token = NULL;
