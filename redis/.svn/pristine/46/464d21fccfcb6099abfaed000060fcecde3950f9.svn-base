...
req_forward(struct context *ctx, struct conn *c_conn, struct msg *msg)
{
    ...
    s_conn = server_pool_conn(ctx, c_conn->owner, key, keylen, msg->type);
    if (s_conn == NULL) {
...
req_judge_perm(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    if (ctx->whitelist == 1) {
        if((msg->type & msg->owner->perm) == 0 && msg->type != 0) {
            conn->done = 1;
            if (pool->perm_warning >= MAX_PERM_WARNING) {
                pool->perm_warning = 0;    
                log_error("The perm is not right!Drop the messages!The ip of client is %s",nc_unresolve_peer_desc(conn->sd));
            } else {
                (pool->perm_warning)++;
            }
            req_put(msg);
              ...
              struct msg *nmsg)
{
    ...
    log_debug(LOG_VERB,"Just 4 test,the type of this message is:%u,type is %d",msg->owner->perm,msg->type);
	...
	if (req_judge_perm(ctx, conn, msg) == false)
		return;
...
req_construct(struct context *ctx, struct conn *conn, char *cmd_str)
{
    ...
    cmd_str_size = nc_strlen(cmd_str);
    do {
        mbuf = STAILQ_LAST(&construct_req->mhdr, mbuf, next);
        if (mbuf == NULL || mbuf_full(mbuf)) {
            ...
            mbuf_insert(&construct_req->mhdr, mbuf);
            construct_req->pos = mbuf->pos;
        }
        ASSERT(mbuf->end - mbuf->last > 0);

        msize = mbuf_size(mbuf);
        copy_size = cmd_str_size <= msize ? cmd_str_size : msize;
        nc_memcpy(mbuf->last, cmd_str, copy_size);

        mbuf->last += copy_size;
        construct_req->mlen += (uint32_t)copy_size;

        cmd_str_size -= copy_size;
        cmd_str += copy_size;
    } while (cmd_str_size > 0);
