...
## Help
    ...
    Usage: nutcracker [-?hVdDt] [-v verbosity level] [-o output file]
                      ...
                      [-i stats interval] [-S sentinel port]
                      [-I server reconnect interval] [-A sentinel addr]
    ...
    Options:
      ...
      -S, --sentinel-port=N     : set sentinel server port (default: 26379)
      -A, --sentinel-addr=S     : set sentinel server ip (default: 127.0.0.1)
      -I, --server-interval=N   : set server reconnect interval in msec (default: 10000 msec)
      -p, --pid-file=S          : set pid file (default: off)
      -m, --mbuf-size=N         : set size of mbuf chunk in bytes (default: 16384 bytes)
      -w, --whitelist_file=S    : set whitelist filename (defaualt :conf/whitelist)
...
For example, the configuration file in [conf/nutcracker.yml](conf/nutcracker.yml), also shown below, configures 6 server pools with names - _alpha_, _beta_, _gamma_, _delta_, _omega_ and sigma. Clients that intend to send requests to one of the 10 servers in pool delta connect to port 22124 on 127.0.0.1. Clients that intend to send request to one of 2 servers in pool omega connect to unix path /tmp/gamma. Requests sent to pool alpha and omega have no timeout and might require timeout functionality to be implemented on the client side. On the other hand, requests sent to pool beta, gamma and delta timeout after 400 msec, 400 msec and 100 msec respectively when no response is received from the server. Of the 6 server pools, only pools alpha, gamma and delta are configured to use server ejection and hence are resilient to server failures. 5 server pools(except sigma) use ketama consistent hashing for key distribution with the key hasher for pools alpha, beta, gamma and delta set to fnv1a_64 while that for pool omega set to hsieh. Server pool sigma use slot for key distribution with the key hasher crc16. Also only pool beta and sigma uses [nodes names](notes/recommendation.md#node-names-for-consistent-hashing) for consistent hashing, while pool alpha, gamma, delta and omega use 'host:port:weight' for consistent hashing. Server pool sigma configured three addresses in each redis sharding to open read write requests spliting. The first address in each sigma sharding will be forwarded write requests, the other two will be forwarded read requests round robin. Finally, only pool alpha, beta and sigma can speak redis protocol, while pool gamma, deta and omega speak memcached protocol.
